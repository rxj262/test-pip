(function ($, Drupal, window, document, undefined) {
  ...
  // TODO: SPLIT CODE TO LOAD IN TWIG VIA LIBRARIES
  ...
  Drupal.behaviors.law = {
    attach: function (context, settings) {
      if (typeof context['location'] !== 'undefined') { // Only fire on document load.
        ...
        $('.field--name-field-lw-hm-hero-text').prepend('<span class="pre-text">Be ready</span>');
        var heroTexts = $('.field--name-field-lw-hm-hero-text .field--item').hide();
        var heroTextCounter = 0;

        function cycleText() {
          heroTexts.eq(heroTextCounter).fadeIn(200)
            .delay(3000)
            .fadeOut(200, cycleText);

          heroTextCounter = ++heroTextCounter % heroTexts.length;
        };
