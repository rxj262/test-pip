...
var data = {};
$('.view-id-campus_map>div.view-content').each(function (i, a) {
  ...
  $(a).find('.views-row').each(function(j,b) {
    var $b = $(b),
        ...
        headline = id + '. ' + title,
        description = $b.find('div.views-field-field-uga-campus-loc-description>div>p').text(),
        lat = $b.find('div.views-field-field-uga-campus-location-lat>div').text().trim(),
        lng = $b.find('div.views-field-field-uga-campus-location-lon>div').text().trim(),
        gallery = $b.find('div.views-field-field-uga-campus-location-ref').find('.slick--main .media__image');
    ...
    for (var i = 0; i < locations.length; i++) {
        ...
        google.maps.event.addListener(marker, 'click', (function(marker, index) {
            return function() {
                ...
                var bumpedlatLng = new google.maps.LatLng(marker.position.lat() - (($(window).width() < 992) ? 0 : ((map.zoom > 15) ? (0.002 / ((map.zoom - 15) * 2)) : 0.002)), marker.position.lng() + (($(window).width() < 992) ? 0 : ((map.zoom > 15) ? (0.01 / ((map.zoom - 15) * 2)) : 0.01)));
...
function showDetail(locations, index) {
    ...
    //     draggable: false
    // });
    (shouldItScroll == 0) ? '' : $('html,body').animate({scrollTop: ($('#map-feature').offset().top - (($(window).width() < 992) ? 300 : 80))});
    shouldItScroll = 1;
}
triggerClick(0);
})(jQuery);
