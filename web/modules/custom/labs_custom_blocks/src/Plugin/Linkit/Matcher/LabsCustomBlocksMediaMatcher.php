 ...
 * )
 */
class LabsCustomBlocksMediaMatcher extends NodeMatcher {
  /**
   *
  ...
  public function execute($string) {
    ...
    if (\Drupal::currentUser()->isAuthenticated() && !$this->currentUser->hasRole('administrator')) {
      $media_to_remove = array();
      // query results are basically node IDs, we can load field_site value and then compare to current user's value.
      foreach ($query_result as $media_id) {
        $media = $this->entityTypeManager->getStorage('media')->load($media_id);
        $entity_term_ids = $this->userSitesInfo->getCurrentEntityTermIds($media);
        $current_user_term_ids = $this->userSitesInfo->getCurrentUserTermIds();
        if (empty(array_intersect($entity_term_ids, $current_user_term_ids))) {
          $media_to_remove[] = $media_id;
        }
      }
      $query_result = array_diff($query_result, $media_to_remove);
    }
